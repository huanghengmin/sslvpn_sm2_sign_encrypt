...
public class TrustCertificateAction extends ActionSupport {
    ...
    public String update() throws Exception {
        ...
        if (null != checks && checks.size() > 0) {
            ...
            for (int i = 0; i < ss.length; i++) {
                String derfile = checks.get(i).getFile();
                String cerfile = CertificateUtils.getFileName(derfile) + X509Context.certName;
                logger.info("cerfile:" + cerfile);
                ss[i] = cerfile;
            }
    ...
    public String upload() throws Exception {
        ...
        if (trustCertificate != null) {
            ...
            String filename = StringContext.server_sslPath + "/" + crtFileFileName;
            String name = CertificateUtils.getFileName(filename);
            boolean crtFlag = FileUtil.saveUploadFile(crtFile, name + X509Context.certName);
            if (crtFlag) {
//                X509CertificateStructure cert = null;
                /*try {
                    cert = CertificateUtils.get_x509_certificate(new File(name + X509Context.certName));
                    if(cert!=null){
                        FileUtil.copy(new File(name + X509Context.certName),name+X509Context.derName);
                    }
                } catch (Exception e) {

                }*/
//                if (cert == null) {
                    X509ShellUtils.build_pem_der(name + X509Context.certName, name + X509Context.derName);
                    SimpleDateFormat format = new SimpleDateFormat("yyyy年MM日dd日HH时mm分ss秒");
                    File file = new File(name + X509Context.derName);
                    if (file.exists()) {
                        X509CertificateStructure cert = CertificateUtils.get_x509_certificate(file);
                        String subject = cert.getSubject().toString();
                        Date notBefore = cert.getStartDate().getDate();//得到开始有效日期
                        Date notAfter = cert.getEndDate().getDate();  //得到截止日期
                        String s_notBefore = "";
                        String s_notAfter = "";
                        if (null != notBefore) {
                        ...
                        tr.setFile(name + X509Context.derName);
...
//                }
    ...
    public String remove() throws Exception {
        ...
        try {
            ...
            String file = trustCertificate.getFile();
            String name = CertificateUtils.getFileName(file);
            File ca_file = new File(name + X509Context.certName);
            if (ca_file.exists()) {
                ca_file.delete();
            }
            File ca_der_file = new File(name + X509Context.derName);
            if (ca_der_file.exists()) {
                ca_der_file.delete();
            }
    ...
    public String download() throws Exception {
        ...
        String name = trustCertificate.getFile();
        String fileName = CertificateUtils.getFileName(name);
        File file = new File(fileName + X509Context.certName);
    ...
    public String findTrustCertificate() throws Exception {
        ...
        SimpleDateFormat format = new SimpleDateFormat("yyyy年MM日dd日HH时mm分ss秒");
        if (f.exists()) {
            X509CertificateStructure cert = CertificateUtils.get_x509_certificate(f);
...
//            }
            ...
            String serialNumber = cert.getSerialNumber().getValue().toString(16).toUpperCase();//得到序列号   16进制
            int version = cert.getVersion();
            String cn = cert.getIssuer().toString();
            json += "{" +
                    "name:'证书版本'," +
                    "content:' V" + version + "'" +
                    "},";
